...
class Config:
    ...
    alpha: float = 0.10
    use_regime_similarity_scoring: bool = True
    ...
    for step, i in enumerate(tqdm(eval_positions, desc=f"Backtest {cfg.symbol}")):
        ...
        if enough_history_for_set:
            ...
            if np.isfinite(regime_z[i]).all():
                if cfg.use_regime_similarity_scoring:
...
def build_ablation_specs() -> Dict[str, Dict[str, object]]:
    return {
        "full": {},
        "no_risk_sensitive_scoring": {
            "lambda_under": 0.0,
        },
        "no_regime_similarity_scoring": {
            "use_regime_similarity_scoring": False,
        },
        "no_high_branch_blend": {
            "high_blend_kappa": 0.0,
        },
        "stress_median_branch": {
            "upper_quantile_q": 0.50,
        },
    }
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ablations", type=str, default="full,no_risk_sensitive_scoring,no_regime_similarity_scoring,no_high_branch_blend,stress_median_branch")
